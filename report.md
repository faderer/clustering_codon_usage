...
得到的结果如下所示，每一行为一个聚类，每一列为该物种类别在某一聚类中出现的次数。可以看到每一个聚类中基本都有某一种物种的集中出现。对聚类结果的更进一步评估将在下一节展开。
...
## 5 心得收获
在此次聚类分析的实验中，我们首先进行了明确的分工，其次学会了如何使用Jupyter Notebook进行数据处理与分析，从而很好地提高了我们的工作效率。在老师课堂讲解的基础上，我们上网搜集了大量聚类算法的原理以及阅读了大量的源代码，对聚类分析相关算法有了更深一步的理解。除此之外，我们对于Python的相关库，如Numpy、Pandas、Sklearn等等，都有了更深一步的认识和掌握。为了获得更好的可视化效果，我们进一步研究了Echarts，并将其运用于我们此次实验中。通过此次实验，我们的学习能力、解决问题的能力、合作能力等等都得到了巨大的提升，感谢老师的指导！
